  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        const response = await fetch(`${env.app.apiUrl}/admin/products/${id}`, {
          method: 'GET',
          headers: {
        ...
        if (data.success && data.data) {
          // Transform additionalInfo from object to Map
          const additionalInfoMap = new Map();
          if (data.data.additionalInfo) {
            Object.entries(data.data.additionalInfo).forEach(([key, value]) => {
              additionalInfoMap.set(key, value);
            });
          }
          
          // Transform categories to array of IDs
          const categoryIds = data.data.categories ? 
            data.data.categories.map(cat => cat._id || cat.id) : [];
          
          // Set product data
          setProduct({
            ...data.data,
            categories: categoryIds,
            primaryCategory: data.data.primaryCategory?._id || data.data.primaryCategory,
            additionalInfo: additionalInfoMap,
            // Ensure medicineDetails has the correct structure
            medicineDetails: {
              manufacturer: data.data.medicineDetails?.manufacturer || '',
              compositions: data.data.medicineDetails?.compositions || []
            },
            // Convert numeric values to strings for form inputs
            stock: data.data.stock?.toString() || '0',
            price: {
              licensedPrice: data.data.price?.licensedPrice?.toString() || '',
              unlicensedPrice: data.data.price?.unlicensedPrice?.toString() || ''
            },
            salePrice: {
              licensedPrice: data.data.salePrice?.licensedPrice?.toString() || '',
              unlicensedPrice: data.data.salePrice?.unlicensedPrice?.toString() || ''
            }
          });
        } else {
        ...
        toast.error('Failed to load product. Redirecting to products page.');
        router.push('/dashboard/products');
      } finally {
